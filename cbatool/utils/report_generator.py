...
class ReportGenerator:
	...
	def create_comprehensive_report(self, 
		...
		# Create individual Excel reports if needed
		report_files = {}
		
		# Generate reports for problem sections
		if 'problem_sections' in analyzer_results and not analyzer_results['problem_sections'].empty:
			problem_sections_path = os.path.join(self.output_directory, "problem_sections.xlsx")
			analyzer_results['problem_sections'].to_excel(problem_sections_path, index=False)
			report_files['Problem Sections'] = problem_sections_path
		
		# Generate reports for anomalies
		if 'anomalies' in analyzer_results and not analyzer_results['anomalies'].empty:
			anomalies_path = os.path.join(self.output_directory, "anomalies.xlsx")
			analyzer_results['anomalies'].to_excel(anomalies_path, index=False)
			report_files['Anomalies'] = anomalies_path
		
		# Check for position analysis results
		if 'position_analysis' in analyzer_results and not analyzer_results['position_analysis'].empty:
			position_path = os.path.join(self.output_directory, "position_analysis.xlsx")
			analyzer_results['position_analysis'].to_excel(position_path, index=False)
			report_files['Position Analysis'] = position_path
		
		# Generate consolidated Excel report
		excel_report = self.consolidate_excel_reports(report_files)
		
		# Generate PDF summary
		summary = analyzer_results.get('summary', {})
		if not summary and 'get_analysis_summary' in dir(analyzer_results):
			# If analyzer_results is actually an Analyzer instance
			summary = analyzer_results.get_analysis_summary()
		
		pdf_report = self.generate_pdf_summary(summary, visualization_path)
